...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.scaling = self.head_dim ** -0.5  # attention计算的缩放尺度
    ...
    def forward(self, x_q: Tensor) -> Tensor:
        ...
        # [N, P, 3, h, c] -> [N, h, 3, P, C]
        qkv = qkv.transpose(1, 3).contiguous()  # 把tensor变成在内存中连续分布的形式
        ...
        attn = torch.matmul(query, key)  # tensor的矩阵点积
        attn = self.softmax(attn)
        attn = self.attn_dropout(attn)  # attention dropout
