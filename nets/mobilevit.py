...
from .mobilevit_utils.model_config import get_config
...
class MobileViT(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        x = self.layer_1(x)  # x(bs, 32, H/2, W/2)
        low_level_features = self.layer_2(x)  # x(bs, 64, H/4, W/4)
        x = low_level_features
        h2, w2 = low_level_features.shape[2], low_level_features.shape[3]  # H/4, W/4
        
        x = self.layer_3(x)  # x(bs, 96, H/8, W/8)
        h3, w3 = x.shape[2], x.shape[3]  # H/8, W/8
        x = self.layer_4(x)  # x(bs, 128, H/16, W/16)
        x = self.layer_5(x)  # x(bs, 160, H/32, W/32)
