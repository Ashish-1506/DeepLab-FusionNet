...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, patch_size=4, in_c=3, embed_dim=96, norm_layer=None):
        super().__init__()
        patch_size = (patch_size, patch_size)  # patch_size = (4, 4)
        self.patch_size = patch_size
        self.in_chans = in_c  # self.in_chans=3
        self.embed_dim = embed_dim  # self.embed_dim=96
        self.proj = nn.Conv2d(
            in_channels=in_c,
    ...
    def forward(self, x):
        _, _, H, W = x.shape  # x(bs, 3, 224, 224)
        ...
        # 下采样patch_size倍
        x = self.proj(x)  # x(bs, 3, 224, 224) -> x(bs, 96, 56, 56)
        _, _, H, W = x.shape
        # flatten: [B, C, H, W] -> [B, C, HW]
        # transpose: [B, C, HW] -> [B, HW, C]
        x = x.flatten(2).transpose(
            1, 2
        )  # x(bs, 96, 56, 56) -> x(bs, 96, 3136) -> x(bs, 3136, 96)
        x = self.norm(x)
        return x, H, W
...
class SwinTransformer(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.patch_embed = PatchEmbed(
            ...
            in_c=in_chans,  # in_chans=3
            embed_dim=embed_dim,  # embed_dim=96
    ...
    def forward(self, x):
        ...
        x, H, W = self.patch_embed(x)  # x(B, 3136, 96), H/4, W/4
        x = self.pos_drop(x)
        ...
        for layer in self.layers:
            x, H, W = layer(x, H, W)
            B = x.shape[0]
            feature_maps = x.permute(0, 2, 1).contiguous().view(B, -1, H, W)
            feature_maps_list.append(feature_maps)

        x = self.norm(x)  # [B, L, C]
        x = x.permute(0, 2, 1).contiguous().view(B, -1, H, W)

        feature_maps_list[0] = F.interpolate(
            input=feature_maps_list[0],
            size=(origin_h // 4, origin_w // 4),
            mode="bilinear",
            align_corners=True,
        )
        x = F.interpolate(
            input=x,
            size=(origin_h // 8, origin_w // 8),
            mode="bilinear",
            align_corners=True,
        )
