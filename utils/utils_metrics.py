...
# Accurate SUIM color-to-class index map
COLOR_MAP = {
    (0, 0, 0): 0,            # BW - Background (000)
    (0, 0, 255): 1,          # HD - Human divers (001)
    (0, 255, 0): 2,          # PF - Aquatic plants (010)
    (0, 255, 255): 3,        # WR - Wrecks/ruins (011)
    (255, 0, 0): 4,          # RO - Robots (100)
    (255, 0, 255): 5,        # RI - Reefs (101)
    (255, 255, 0): 6,        # FV - Fish (110)
    (255, 255, 255): 7       # SR - Sea-floor (111)
}
...
def rgb_to_label(mask):
    ...
    for rgb, cls in COLOR_MAP.items():
        label[np.all(mask == rgb, axis=-1)] = cls
    return label
...
def compute_mIoU(gt_dir, pred_dir, png_name_list, num_classes, name_classes=None):
    ...
    for ind in range(len(gt_imgs)):
        ...
        if pred.shape != label.shape:
            pred = Image.fromarray(pred).resize(label.shape[::-1], Image.NEAREST)
            pred = np.array(pred)
